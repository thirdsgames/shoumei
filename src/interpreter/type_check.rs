...
pub enum Constraint {
    ...
    /// The given type is exactly equal to a function from the left type to the right type.
    EqualityFunction {
        left: TypeVariable,
        right: TypeVariable,
    },
...
impl<'a> TypeChecker<'a> {
    ...
    fn validate_case(
        ...
        arg_vars
            .bind(|arg_vars| {
                self.parse_expr(&arg_vars, HashMap::new(), HashMap::new(), replacement)
                    .deny()
                    .bind(|expr| {
                        println!("Assumptions: {:#?}", expr.assumptions);
                        deduce_expr_type(
                            self.module_path,
                            self.project_index,
                            &arg_vars,
    ...
    fn parse_expr(
        ...
        match expr {
            ExpressionP::Variable(identifier) => {
                ...
                match resolve_symbol(self.module_path, &identifier, self.project_index)
                    .destructure()
                    .0
                {
            ...
            ExpressionP::Apply(left, right) => {
                ...
                .bind(|left| {
                    self.parse_expr(args, monotype_variables, polytype_variables, *right)
                        .map(|right| {
            ...
            ExpressionP::Lambda { params, expr, .. } => {
                ...
                DiagnosticResult::ok_with_many(monotype_variables, messages)
                    ...
                    .bind(|monotype_variables| {
                        self.parse_expr(args, monotype_variables, polytype_variables, *expr)
                            .map(|mut expr| {
                                let type_variable =
                                    TypeVariable::Unknown(TypeVariableId::default());
                                ...
                                for param in param_types.iter().rev() {
                                    let lambda_step_type =
                                        TypeVariable::Unknown(TypeVariableId::default());
                                    expr.constraints.0.push((
                                        lambda_step_type.clone(),
                                        Constraint::EqualityFunction {
                                            left: TypeVariable::Unknown(*param),
                                            right: lambda_type,
                                        },
                                    ));
                                ...
                                expr.constraints.0.push((
                                    type_variable.clone(),
                                    Constraint::Equality(lambda_type),
                                ));
                                    ...
                                    param_assumptions.into_iter().zip(param_types)
                                {
                                    for assumption in assumptions {
                                        expr.constraints.0.push((
                                            ...
                                            Constraint::Equality(TypeVariable::Unknown(
                                                assumption.0,
                                            )),
                                        ));
                                    }
                ...
                .bind(|left_expr| {
                    ...
                    DiagnosticResult::ok_with_many((), messages).bind(|_| {
                        ...
                        .map(|mut right_expr| {
                            ...
                            // Constraints: ts === input_types, for all assumptions that params : ts
                            let mut constraints =
                                left_expr.constraints.union(right_expr.constraints);
