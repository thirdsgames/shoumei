...
fn contains_id(v: &TypeVariable, id: &TypeVariableId) -> bool {
    match v {
        ...
        TypeVariable::Variable(_) => false,
        TypeVariable::Unknown(other_id) => other_id == id,
    }
}
    ...
    match reason {
        ...
        ConstraintEqualityReason::LambdaParameter {
            ...
            let messages = vec![HelpMessage {
                ...
                help_type: HelpType::Note,
                diagnostic: Diagnostic::at(module_path.clone(), param_range),
            }];
            (lambda, messages)
        }
        _ => {
...
fn process_infinite_type_error(
    ...
    let (error_range, help) = process_constraint_reason(module_path, &mut ty_printer, reason);

    ErrorMessage::new_with_many(
        format!(
            "a self-referential type {} ~ {} was created",
            ty_printer.print(TypeVariable::Unknown(error.erroneous_substitutions[0].0)),
